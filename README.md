...
- `glibc_e2e` / `bionic_e2e`: EH fixture plus all realib suites

Use `layer=unit` for fast Rust-only validation or `layer=e2e` when the unit
cache is already warm. The realib runner builds each test target once with
`--no-run` and executes the resulting test binary directly; it does not first
compile the entire workspace and then precompile the same targets again.
...
| `oracle_litmus` | Optional Cat 2B differential-oracle litmus on bionic |
